...
public class CrawlingServiceImpl implements CrawlingService {
    ...
    public void process(Long sourceId) {
        try {
            ...
            var source = techStackSourceRepository.findById(sourceId)
            ...
            var latestVersionOpt = mavenClient.fetchLatestVersion(source.getMavenGroupId(), source.getMavenArtifactId());
            if (latestVersionOpt.isEmpty()) {
                log.warn("최신 버전을 찾을 수 없어 크롤링 건너뜀 sourceId={}", sourceId);
                return;
            }

            var latestVersion = latestVersionOpt.get();
            var techStack = source.getTechStack();
            if (latestVersion.equals(techStack.getLatestVersion())) {
                log.info("신규 버전 없음 sourceId={} latestVersion={}", sourceId, latestVersion);
                return;
            }

            var releaseNoteOpt = githubClient.fetchReleaseNote(source.getGithubOwner(), source.getGithubRepo(), latestVersion);
            if (releaseNoteOpt.isEmpty()) {
                log.warn("릴리즈 노트를 찾을 수 없어 크롤링 건너뜀 sourceId={} version={} ", sourceId, latestVersion);
                return;
            }

            var releaseNote = releaseNoteOpt.get();
            var record = releaseRecordRepository.save(ReleaseRecord.builder()
                    .techStack(techStack)
            ...
            var tags = releaseParser.extractTags(releaseNote.content());
